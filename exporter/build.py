...
FTE = ["engineOUAR.py", "pather.py", "UniPy.py", f"objects{pt.s}object.py", f"objects{pt.s}text.py", f"objects{pt.s}camera.py", f"exporter{pt.s}engine.py", f"exporter{pt.s}settings.py"]
...
def replaceator(string, input_str, num_lines, DMS = False, endStr = "#!&$?", md = 0):
    tbs = 0
    nm = 0
    is_del = False
    out = ""
    ...
    for idx, s in enumerate(input_str.split("\n")):
        add = True
        IF = s.strip() == string if md == 0 else s.strip().startswith(string)
        if IF and not is_del:
            if not DMS: out += s + "\n"
            nm = 0
            is_del = True
            tbs = 0
...
def createProject(create_path, path_to_project, project_name, program_name, program_size, program_version):
    ...
    for file in os.listdir(f"{create_path}{pt.s}{project_name}{pt.s}engine"):
        ...
        if file not in ["engine.py", "engineIcon.png"]:
            ...
            if file in ["engineOUAR.py", "UniPy.py"]: inf = inf.replace("objects", "engine", 2)
            f.close()
