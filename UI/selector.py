...
class Selector:
    def __init__(self, win, func, **args):
        ...
        self.textCentering = args.get("textCentering", "left")
        self.elemsWDiv = args.get("elemsWDiv", 1)
        self.elemsHDiv = args.get("elemsHDiv", 10)
        self.normalize()
    
    def normalize(self):
        self.elem_name = self.elements[:]
        self.elem_width = self.surface.get_width() // self.elemsWDiv
        self.elem_height = self.surface.get_height() // self.elemsHDiv
        ...
        if self.render:
            ...
            for idx, i in enumerate(self.elem_name):
                ...
                if self.textCentering == "center":
                    ...
                    xPos = self.posX + 10
                self.surface.blit(tx, (xPos, yPos + self.oY))
                y += self.elem_height + 10
